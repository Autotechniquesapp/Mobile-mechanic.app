...
list.innerHTML=(data.integrations||[]).map(i=>`<div class="integration-row"><span class="integration-dot ${esc(i.status)}"></span><div><b>${esc(i.display_name)}</b><div class="muted">${esc(String(i.status||'').replaceAll('_',' '))} · ${esc(i.mode||'')}</div><div class="integration-cost-note">${esc(detail(i))}</div>${setupLink(i)}</div></div>`).join('')||'<div class="admin-card muted">No integration records.</div>';
}
