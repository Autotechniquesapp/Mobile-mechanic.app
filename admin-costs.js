...
function detail(i){
  ...
  if(i.provider==='youtube')return 'Optional repair-video lookup · API quota setup still pending';
...
list.innerHTML=(data.integrations||[]).map(i=>`<div class="integration-row"><span class="integration-dot ${esc(i.status)}"></span><div><b>${esc(i.display_name)}</b><div class="muted">${esc(String(i.status||'').replaceAll('_',' '))} · ${esc(i.mode||'')}</div><div class="integration-cost-note">${esc(detail(i))}</div></div></div>`).join('')||'<div class="admin-card muted">No integration records.</div>';
}
...
styles();new MutationObserver(()=>{if(document.querySelector('.integration-list'))setTimeout(load,40);}).observe(document.documentElement,{childList:true,subtree:true});setInterval(load,30000);setTimeout(load,500);
})();