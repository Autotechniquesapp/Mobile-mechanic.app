...
list.innerHTML=(data.integrations||[]).map(i=>`<details class="integration-row"><summary><span class="integration-dot ${esc(i.status)}"></span><div><b>${esc(i.display_name)}</b><div class="muted">${esc(String(i.status||'').replaceAll('_',' '))} · ${esc(i.mode||'')}</div><div class="integration-cost-note">${esc(detail(i))}</div></div></summary><div class="integration-admin-detail"><b>Status:</b> ${esc(String(i.status||'unknown').replaceAll('_',' '))}<br><b>Mode:</b> ${esc(i.mode||'not set')}<br><b>Last checked:</b> ${i.last_checked_at?esc(new Date(i.last_checked_at).toLocaleString()):'Never'}<br><b>Problem:</b> ${esc(i.last_error||'No reported error')}${setupLink(i)}</div></details>`).join('')||'<div class="admin-card muted">No integration records.</div>';
}
