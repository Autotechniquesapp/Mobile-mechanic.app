...
function render(data){
  ...
  <aside class="side-stack"><section class="section admin-panel"><div class="panel-head"><div><h2>Integrations</h2><p>Stripe, email, and SMS setup.</p></div></div><div class="integration-list">${(data.integrations||[]).length?(data.integrations||[]).map(i=>`<div class="integration-row"><span class="integration-dot ${esc(i.status)}"></span><div><b>${esc(i.display_name)}</b><div class="muted">${esc(i.status.replace('_',' '))} · ${esc(i.mode)}</div></div></div>`).join(''):'<div class="admin-card muted">Integration setup records are not loaded yet.</div>'}</div></section>
