...
function shopRow(s,owner){
  const protectedShop=Boolean(s.comped_permanent);
  const source=s.referral_code||s.referred_by_name||'—';
  const planNames={solo:'Solo',shop:'Shop',pro_fleet:'Pro'};
  return `<tr data-shop-status="${esc(s.billing_status)}" data-shop-id="${esc(s.shop_id)}" data-protected="${protectedShop?'true':'false'}"><td class="check-cell" data-label="Select"><input class="admin-check shop-check" type="checkbox" data-shop="${esc(s.shop_id)}" ${protectedShop?'disabled title="Protected permanent-comp shop"':''}></td><td data-label="Shop"><div class="shop-name">${esc(s.name)}</div>${protectedShop?'<div class="owner-shop">Permanent comp · protected</div>':''}<div class="shop-slug">${esc(s.slug)}</div></td><td data-label="Referral">${esc(source)}</td><td data-label="Plan">${owner&&!protectedShop?`<select class="admin-select plan-select" data-shop="${esc(s.shop_id)}"><option value="solo" ${s.plan==='solo'?'selected':''}>Solo · $29.99</option><option value="shop" ${s.plan==='shop'?'selected':''}>Shop · $69.99</option><option value="pro_fleet" ${s.plan==='pro_fleet'?'selected':''}>Pro · $129.99</option></select>`:`${esc(planNames[s.plan]||s.plan)}`}</td><td data-label="Status"><span class="status ${esc(s.billing_status)}">${esc(s.billing_status)}</span></td><td data-label="Trial">${protectedShop?'Non-expiring':date(s.trial_expires_at)}</td><td data-label="Users">${Number(s.active_members||0)}</td><td data-label="Actions"><div class="admin-actions">${owner&&!protectedShop?`<button class="admin-btn" data-admin-action="extend" data-shop="${esc(s.shop_id)}">+30 Days</button>${s.billing_status==='suspended'?`<button class="admin-btn" data-admin-action="status" data-status="active" data-shop="${esc(s.shop_id)}">Reactivate</button>`:`<button class="admin-btn danger" data-admin-action="status" data-status="suspended" data-shop="${esc(s.shop_id)}">Suspend</button>`}<button class="admin-btn" data-admin-action="status" data-status="comped" data-shop="${esc(s.shop_id)}">Comp</button>`:'<span class="muted">Protected</span>'}</div></td></tr>`;
}
...
async function loadOverview(){
  try{
    const data=await call({action:'overview'});
    const {data:integrations}=await sb.from('platform_integrations').select('provider,display_name,status,mode,last_checked_at,last_error').order('provider');
    const {data:templates}=await sb.from('message_templates').select('template_id,channel,name,active').eq('active',true).order('template_id');
