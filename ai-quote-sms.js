...
.aiq-main-mic{width:100%;min-height:116px;margin:12px 0;display:flex;flex-direction:column;gap:6px;font-size:15px;border:1px solid #ef2a31;background:radial-gradient(circle at 50% 20%,#5b191e,#231014 58%,#11161c);color:#fff;border-radius:16px;box-shadow:0 0 24px rgba(239,42,49,.18)}.aiq-main-mic .aiq-mic-icon{font-size:32px}.aiq-helpbox{margin-top:10px;padding:12px;border:1px solid #343d47;border-radius:10px;background:#0b0f14}.aiq-helpbox li{margin:7px 0;color:#c8ced6}
...
function panelMarkup(job){return `<section class="aiq-panel" data-aiq-panel>
  <div class="aiq-head"><div><b>QUICK QUOTE</b><br><span>Speak naturally. Review every draft before sending.</span></div><button class="btn btn-soft" type="button" data-aiq-help>What can I say?</button></div>
  <button class="aiq-main-mic" type="button" data-aiq-mic><span class="aiq-mic-icon">🎤</span><b>START TALKING</b><span>Describe the repair, parts, labor, fees, deposit, and warranty.</span></button>
  <div class="field"><label>What the mechanic said</label><textarea data-aiq-notes placeholder="Example: Replace front pads and rotors. Parts cost $280. Charge 3 hours labor and add a $50 service call."></textarea><div class="aiq-voice-status" data-aiq-voice-status>Tap the microphone and speak normally.</div></div>
  <input type="hidden" data-aiq-parts value="">
  <div data-aiq-helpbox></div>
...
async function sendSms(to,text){try{const d=await invoke('sms-send',{action:'send',to,text});toast('SMS sent to customer.','good');return {sent:true,provider:'twilio',data:d};}catch(err){nativeSms(to,text);toast('Automatic SMS is unavailable or the shop has no SMS pack remaining, so I opened your phone Messages app with the text filled in.','');return {sent:false,provider:'device',error:err?.message||''};}}
...
document.addEventListener('click',e=>{if(e.target.closest('[data-aiq-help]')){e.preventDefault();const box=document.querySelector('[data-aiq-helpbox]');if(box)box.innerHTML=box.innerHTML?'':`<div class="aiq-helpbox"><b>Try saying:</b><ul><li>Replace front brake pads and rotors.</li><li>Parts cost $280 with a 30% markup.</li><li>Charge three hours labor at $120 per hour.</li><li>Add a $50 mobile service fee.</li><li>Require parts cost as the deposit.</li><li>Change labor to four hours.</li><li>Remove the diagnostic fee.</li></ul></div>`;return;}if(e.target.closest('[data-aiq-edit-voice]')){e.preventDefault();startVoice();voiceStatus('Tell me what to add, remove, or change. Your existing draft stays until you generate the update.');return;}if(e.target.closest('[data-aiq-mic]')){e.preventDefault();startVoice();return;}if(e.target.closest('[data-aiq-generate]')){e.preventDefault();generate();return;}if(e.target.closest('[data-aiq-save]')){e.preventDefault();saveOnly();return;}if(e.target.closest('[data-aiq-save-text]')){e.preventDefault();saveAndText();return;}if(e.target.closest('[data-aiq-text-customer]')){e.preventDefault();quickText();}},true);
