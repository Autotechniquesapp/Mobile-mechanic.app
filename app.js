...
const plans = {
  ...
  pro:  {name:'Pro', price:129.99, seats:15, label:'Larger shop'}
};
...
function checkedCards(name,items,selected=[]){
  return `<div class="plan-cards config-cards">${items.map(([code,label])=>`<label class="plan-card config-toggle"><input type="checkbox" name="${name}" value="${code}" ${selected.includes(code)?'checked':''}><b>${esc(label)}</b><span class="toggle-state"><i class="state-on">ON</i><i class="state-off">OFF</i></span></label>`).join('')}</div>`;
}
...
const platformPerms={
  platform_owner:new Set(['shops_view','shops_open','trial_extend','comp','suspend','activity_view','billing_view','tech_view']),
...
function platformUser(){ return db.session?.role==='platform_owner'?db.platformOwner:null; }
...
function topbar(s,active='dashboard'){
  ...
  return `<header class="topbar">
    ...
    <div class="brand" data-route="dashboard">${logo(s,'brand-mark')}<div class="brand-copy"><div class="brand-title">MOBILE <span class="red">MECHANIC</span> AI <em class="app-version">V3</em></div><div class="brand-sub">Work Smarter. Fix Faster. Get Paid.</div></div></div>
...
function mobileDrawer(s,active){
  ...
  return `<div class="drawer-backdrop" data-action="close-menu" aria-hidden="true"></div><aside class="mobile-drawer" aria-hidden="true" aria-label="Main navigation"><div class="drawer-head">${logo(s)}<div><b>Mobile Mechanic AI <em class="app-version">V3</em></b><span>${esc(role)}</span></div><button data-action="close-menu" aria-label="Close navigation">×</button></div><nav>${links.map(([r,i,t])=>`<button class="drawer-link ${active===r?'active':''}" data-route="${r}">${ic(i)}<span>${t}</span><strong>›</strong></button>`).join('')}</nav><div class="drawer-account"><b>${esc(s.name)}</b><span>${esc(currentUser()?.name||'Technician')} · ${plans[s.plan]?.name||''}</span></div></aside>`;
}
...
function inspection(){
  const s=currentShop();
  const activeJob=s.jobs.find(x=>x.id===db.session.activeJobId),draft=activeJob?.inspectionDraft||{};
  const content=`${pageTitle('Pre-Purchase Inspection','Customer flow, required photos, seller contact, scan results, and AI summary.')}
  <section class="work-white"><div class="work-card"><h3>Inspection Request</h3><div class="row2"><div class="field"><label>Customer Name</label><input id="ppiCustomer" value="${esc(draft.customerName||activeJob?.customerName||'')}"></div><div class="field"><label>Seller / Vehicle Owner</label><input id="ppiSeller" value="${esc(draft.seller||'')}"></div></div><div class="row3"><div class="field"><label>Year</label><select id="ppiYear"><option>${esc(draft.year||activeJob?.vehicle?.year||new Date().getFullYear())}</option>${yearOptions()}</select></div><div class="field"><label>Make</label><input id="ppiMake" value="${esc(draft.make||activeJob?.vehicle?.make||'')}"></div><div class="field"><label>Model</label><input id="ppiModel" value="${esc(draft.model||activeJob?.vehicle?.model||'')}"></div></div><div class="row2"><div class="field"><label>VIN</label><input id="ppiVin" value="${esc(draft.vin||activeJob?.vehicle?.vin||'')}"></div><div class="field"><label>Mileage</label><input id="ppiMiles" type="number" value="${esc(draft.mileage||activeJob?.vehicle?.mileage||'')}"></div></div></div>
  <div class="work-card" style="margin-top:8px"><h3>Inspection Checklist</h3><div class="row2">${['Exterior / Body','Tires / Wheels','Brakes','Suspension / Steering','Engine / Fluids','Transmission / Driveline','Electrical / Battery','HVAC','Interior','Scan All Modules','Road Test','Safety Concerns'].map(x=>`<label class="diag-check"><input type="checkbox" data-ppi-item value="${esc(x)}" ${(draft.checked||[]).includes(x)?'checked':''}>${x}</label>`).join('')}</div></div><div class="work-card" style="margin-top:8px"><h3>Required Photos</h3><div class="finding-photo-grid">${Array.from({length:6},(_,i)=>`<label class="photo-placeholder" title="Inspection photo ${i+1}">${ic('camera')}<input type="file" data-ppi-photo accept="image/*" capture="environment" hidden></label>`).join('')}</div></div><div class="work-card" style="margin-top:8px"><h3>Inspection Notes</h3><div class="field"><textarea id="ppiNotes" placeholder="Condition, measurements, concerns, and recommendations">${esc(draft.notes||'')}</textarea></div></div><div class="work-card" style="margin-top:8px"><h3>AI Customer Summary</h3><p class="muted">Technician findings can be converted into Good / Monitor / Needs Attention / Safety Concern categories. Technician reviews before sending.</p><button class="btn btn-primary" data-action="save-ppi">Save Inspection Draft</button></div></section>`;
...
function settings(){
  ...
  <form id="businessTypesForm"><section class="card card-pad"><div class="card-title">BUSINESS TYPES & TERMINOLOGY</div><div class="section-note">Tap anywhere on a card to switch it ON or OFF. Existing records are kept.</div><div class="divider"></div>${checkedCards('specialties',repairSpecialties,s.specialties)}<div class="row2" style="margin-top:10px"><div class="field"><label>Custom Specialty</label><input name="customSpecialty" value="${esc(s.customSpecialty)}" placeholder="Optional"></div><div class="field"><label>Unit Name</label><select name="assetLabel">${['Vehicle / Equipment','Vehicle','Equipment','Unit','Machine','Asset'].map(x=>`<option ${s.assetLabel===x?'selected':''}>${x}</option>`).join('')}</select></div></div></section><section class="card card-pad" style="margin-top:10px"><div class="card-title">OPTIONAL MODULES</div><div class="section-note">Tap the whole card to switch a tool ON or OFF, then save. Turning it off hides the tool without erasing its data.</div><div class="divider"></div>${checkedCards('modules',shopModules,s.modules)}<button class="btn btn-primary btn-wide" type="submit" style="margin-top:12px">Save Business Types & Tools</button></section></form>
  ...
  <section class="card card-pad" style="margin-top:10px"><div class="card-title">ACCOUNT SECURITY</div><div class="section-note">Change the password for your signed-in account.</div><div class="divider"></div><form id="changePasswordForm"><div class="field"><label>Current Password</label><input name="currentPassword" type="password" autocomplete="current-password" required></div><div class="row2"><div class="field"><label>New Password</label><input name="newPassword" type="password" minlength="8" autocomplete="new-password" required></div><div class="field"><label>Confirm New Password</label><input name="confirmPassword" type="password" minlength="8" autocomplete="new-password" required></div></div><button class="btn btn-primary" type="submit">Change My Password</button></form></section>
...
function reportJobFacts(s,j){
  const declinedIds=new Set((s.declined||[]).map(x=>String(x.jobId||x.id||'')));
  const warrantyIds=new Set((s.warranties||[]).map(x=>String(x.jobId||x.originalJobId||x.id||'')));
...
function fleet(){ simpleModule('Fleet Service','Pro tools for fleet customers, unit numbers, diesel/semi service, maintenance history, and roadside.',[['truck','Fleet Units','Multiple vehicles, unit numbers, driver contacts, locations, and maintenance records.'],['brain','Fleet AI Intake','Vehicle/unit-specific complaint and diagnostic pre-workup.'],['calendar','Scheduled Maintenance','Mileage/time-based maintenance recommendations and reminders.'],['report','Fleet Reports','Service history and recurring maintenance records.']]); }
...
function platformAdmin(){
  ...
  const canAct=platformCan('activity_view');
  ...
  <section class="card card-pad" style="margin-top:10px"><div class="card-title">PLATFORM OWNER</div><div class="section-note">This platform has one owner account. Shop owners and managers only administer their own shop.</div><div class="divider"></div><div class="list">${admins.map(a=>`<div class="list-item"><div class="list-icon">${ic('shield')}</div><div class="list-main"><b>${esc(a.name)}</b><p>${esc(a.email)} • Platform Owner • ${a.active?'Active':'Disabled'}</p></div></div>`).join('')}</div></section>
...
function bind(){
  ...
  document.querySelector('[data-action="scan-placeholder"]')?.addEventListener('click',()=>{const input=document.createElement('input');input.type='file';input.accept='image/*';input.capture='environment';input.onchange=async()=>{const file=input.files?.[0];if(!file)return;try{if(!('BarcodeDetector' in window))throw new Error('recognizer');const bitmap=await createImageBitmap(file),codes=await new BarcodeDetector().detect(bitmap),vin=codes.map(x=>String(x.rawValue||'').toUpperCase().replace(/[^A-Z0-9]/g,'')).find(x=>/^[A-HJ-NPR-Z0-9]{17}$/.test(x));bitmap.close?.();if(!vin)throw new Error('recognizer');const field=document.getElementById('vinInput');if(field){field.value=vin;field.dispatchEvent(new Event('input',{bubbles:true}));}toast(`VIN captured: ${vin}`,'good');}catch{toast('Photo captured, but the VIN was not recognized. Enter it manually from the photo.','');}};input.click();});
  ...
  document.querySelector('[data-module="ai"][data-action="ask-vehicle"]')?.addEventListener('click',e=>{db.session.activeJobId=e.currentTarget.dataset.job;save();const help=window.MobileMechanicTechnicianHelp;if(help?.open)return help.open();toast('Technician Help is unavailable. Reload the app and try again.','bad');});
  ...
  document.querySelectorAll('[data-action="reset-tech-password"]').forEach(b=>b.onclick=()=>{const s=currentShop(),u=s.users.find(x=>x.id===b.dataset.user);modal('Reset Technician Password',`<form id="resetPassForm" data-user="${u.id}"><p class="small muted">Send a secure password-reset link to <b>${esc(u.email||'this technician')}</b>. The shop never sees or stores the technician's password.</p><button class="btn btn-primary" type="submit">Send Password Reset Email</button></form>`);});
  document.getElementById('resetPassForm')?.addEventListener('submit',e=>{e.preventDefault();if(!window.MobileMechanicSupabase)toast('Secure password reset is temporarily unavailable. Try again after reconnecting.','bad');});
  ...
  document.querySelector('[data-action="save-ppi"]')?.addEventListener('click',()=>{const s=currentShop(),j=jobById(db.session.activeJobId);if(!j)return toast('Open a job before saving the inspection.','bad');const checked=[...document.querySelectorAll('[data-ppi-item]:checked')].map(x=>x.value),photos=[...document.querySelectorAll('[data-ppi-photo]')].filter(x=>x.files?.length).map(x=>({name:x.files[0].name,size:x.files[0].size,type:x.files[0].type}));j.inspectionDraft={customerName:document.getElementById('ppiCustomer')?.value.trim()||'',seller:document.getElementById('ppiSeller')?.value.trim()||'',year:document.getElementById('ppiYear')?.value||'',make:document.getElementById('ppiMake')?.value.trim()||'',model:document.getElementById('ppiModel')?.value.trim()||'',vin:document.getElementById('ppiVin')?.value.trim().toUpperCase()||'',mileage:document.getElementById('ppiMiles')?.value||'',checked,notes:document.getElementById('ppiNotes')?.value.trim()||'',photos,savedAt:nowISO(),savedBy:currentUser()?.id||null};s.inspections=Array.isArray(s.inspections)?s.inspections:[];const old=s.inspections.findIndex(x=>x.jobId===j.id);const record={jobId:j.id,...j.inspectionDraft};if(old>=0)s.inspections[old]=record;else s.inspections.push(record);save();toast(`Inspection draft saved (${checked.length} checks, ${photos.length} photos selected).`,'good');});
...
document.addEventListener('click',e=>{
  ...
  if(reportFilter){e.preventDefault();reports(reportFilter.dataset.reportFilter);return;}
  const intakeAction=e.target.closest?.('[data-action="share-intake"],[data-action="copy-intake"],[data-action="preview-intake"]');
  if(intakeAction){
    e.preventDefault();
    const s=currentShop();
    if(!s)return toast('Open a shop before using the intake link.','bad');
    if(intakeAction.dataset.action==='preview-intake')return publicIntake(s);
    const url=intakeUrl(s);
    if(intakeAction.dataset.action==='share-intake'&&navigator.share){
      navigator.share({title:`${s.name} Customer Intake`,text:`Please fill out this vehicle intake for ${s.name}.`,url}).catch(()=>{});
      return;
    }
    if(navigator.clipboard?.writeText)navigator.clipboard.writeText(url).then(()=>toast('Shop intake link copied.','good')).catch(()=>toast('Press and hold the link above to copy it.','bad'));
    else toast('Press and hold the link above to copy it.','bad');
    return;
  }
...
function render(route=hashRoute()){
  ...
  if(route==='login')return login();if(route==='signup')return signup();if(route==='admin'){if(db.session?.role==='platform_owner')return platformAdmin();return login();}
...
if(intakeSlug){
  ...
  if(!location.hash) location.hash=db.session?.role==='platform_owner'?'#admin':db.session?.role==='shop'?'#dashboard':'#login';
  render();
}
