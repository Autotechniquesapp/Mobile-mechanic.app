...
.mma-day-open{width:100%;border:0;background:transparent;color:#fff;text-align:left;padding:0}.mma-day-open .mma-day-head{pointer-events:none}.mma-empty-day{width:100%;min-height:62px;margin-top:8px;border:1px dashed #3a444f;border-radius:10px;background:#0f141a;color:#aeb6c0;text-align:left;padding:12px}
.mma-cal-job-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}.mma-cal-job-actions button{min-height:38px;border-radius:9px;border:1px solid #303944;background:#11161c;color:#fff;padding:8px 10px;font-weight:800}
...
function enhanceCalendar(){const title=[...document.querySelectorAll('.page-title h2')].find(x=>x.textContent.trim()==='Calendar');if(!title||document.querySelector('[data-mma-calendar-enhanced]'))return;const scheduledSec=[...document.querySelectorAll('.content section')].find(sec=>String(sec.querySelector('.card-title')?.textContent||'').includes('SCHEDULED JOBS'));if(!scheduledSec)return;const list=scheduledSec.querySelector('.list');const nodes=new Map();[...list?.children||[]].forEach(node=>{const b=node.querySelector('[data-action="schedule-job"][data-job]');if(b)nodes.set(b.dataset.job,node);});const stash=document.createElement('div');stash.style.display='none';scheduledSec.appendChild(stash);moveAllBack(stash,nodes);scheduledSec.style.display='none';const panel=document.createElement('section');panel.className='card card-pad mma-calendar-panel';panel.dataset.mmaCalendarEnhanced='1';scheduledSec.insertAdjacentElement('beforebegin',panel);
 ...
 function openDay(date){document.querySelector(`[data-calendar-day="${dayKey(new Date(date))}"]`)?.click();}
 function decorateJobNode(node,j){if(!node||node.querySelector('[data-cal-open-job]'))return;const actions=node.querySelector('.list-actions')||node.querySelector('.list-main');if(!actions)return;actions.insertAdjacentHTML('afterbegin',`<button type="button" class="btn btn-primary" data-cal-open-job="${esc(j.id)}">Open Job</button>`);}
 function renderWeek(){moveAllBack(stash,nodes);const start=mondayOf(calendarAnchor);panel.innerHTML=`<div class="mmp-section-head"><b>WEEK OF ${esc(start.toLocaleDateString([], {month:'short',day:'numeric'}))}</b><span class="muted small">Every row is clickable</span></div>${controls()}<div class="mma-week-grid" data-week-grid></div>`;const grid=panel.querySelector('[data-week-grid]');for(let n=0;n<7;n++){const d=addDays(start,n),key=dayKey(d),box=document.createElement('div');box.className='mma-week-day'+(sameDay(d,new Date())?' today':'');box.innerHTML=`<button type="button" class="mma-day-open" data-cal-open-day="${key}"><div class="mma-day-head"><b>${esc(fmtDay(d))}</b><span class="muted small">Open day</span></div></button><div class="list"></div>`;const dest=box.querySelector('.list'),dayJobs=all().filter(j=>sameDay(new Date(j.scheduledStart),d));if(dayJobs.length){dayJobs.forEach(j=>{const node=nodes.get(j.id);decorateJobNode(node,j);if(node)dest.appendChild(node);});}else dest.innerHTML=`<button type="button" class="mma-empty-day" data-cal-open-day="${key}">No jobs. Tap to add or schedule one.</button>`;grid.appendChild(box);}bindControls();}
 function renderMonth(){moveAllBack(stash,nodes);const first=monthStart(calendarAnchor),gridStart=addDays(first,-first.getDay()),month=first.getMonth();panel.innerHTML=`<div class="mmp-section-head"><b>${esc(first.toLocaleDateString([], {month:'long',year:'numeric'}).toUpperCase())}</b><span class="muted small">Tap any day or job</span></div>${controls()}<div class="mma-month-scroll"><div class="mma-month-grid" data-month-grid>${['SUN','MON','TUE','WED','THU','FRI','SAT'].map(x=>`<div class="mma-month-weekday">${x}</div>`).join('')}</div></div>`;const grid=panel.querySelector('[data-month-grid]'),scheduled=all();for(let n=0;n<42;n++){const d=addDays(gridStart,n),key=dayKey(d),cell=document.createElement('button');cell.type='button';cell.className='mma-month-day'+(d.getMonth()!==month?' muted':'')+(sameDay(d,new Date())?' today':'');cell.dataset.calOpenDay=key;const js=scheduled.filter(j=>sameDay(new Date(j.scheduledStart),d));cell.innerHTML=`<strong>${d.getDate()}</strong>${js.map(j=>`<span class="mma-month-job" data-cal-open-date="${d.toISOString()}">${esc(fmtTime(j.scheduledStart))} ${esc(j.customerName||'Customer')}</span>`).join('')}`;grid.appendChild(cell);}bindControls();}
 function render(){calendarMode==='month'?renderMonth():renderWeek();}
 function bindControls(){panel.querySelector('[data-cal-prev]')?.addEventListener('click',()=>{calendarAnchor=calendarMode==='month'?new Date(calendarAnchor.getFullYear(),calendarAnchor.getMonth()-1,1):addDays(calendarAnchor,-7);render();});panel.querySelector('[data-cal-next]')?.addEventListener('click',()=>{calendarAnchor=calendarMode==='month'?new Date(calendarAnchor.getFullYear(),calendarAnchor.getMonth()+1,1):addDays(calendarAnchor,7);render();});panel.querySelector('[data-cal-today]')?.addEventListener('click',()=>{calendarAnchor=new Date();render();openDay(calendarAnchor);});panel.querySelectorAll('[data-cal-mode]').forEach(b=>b.addEventListener('click',()=>{calendarMode=b.dataset.calMode;render();}));panel.querySelectorAll('[data-cal-open-day]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();openDay(b.dataset.calOpenDay);}));panel.querySelectorAll('[data-cal-open-job]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();location.hash=`#findings?id=${encodeURIComponent(b.dataset.calOpenJob)}`;}));panel.querySelectorAll('[data-cal-open-date]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();calendarAnchor=new Date(b.dataset.calOpenDate);calendarMode='week';render();setTimeout(()=>panel.scrollIntoView({behavior:'smooth',block:'start'}),20);}));}
 render();}
