...
async function shareIntake(){const shop=currentShop();if(!shop?.slug)return toast('Shop intake link is not ready.','bad');const url=intakeUrl(shop);const text=`Please fill out this vehicle intake for ${shop.name||'the shop'}:\n${url}`;try{if(navigator.share){await navigator.share({title:`${shop.name||'Shop'} Customer Intake`,text,url});}else if(navigator.clipboard){await navigator.clipboard.writeText(text);toast('Intake link copied.','good');}else{location.href=`sms:?body=${encodeURIComponent(text)}`;}}catch(err){if(err?.name!=='AbortError'){try{await navigator.clipboard?.writeText(text);toast('Intake link copied.','good');}catch{}}}}
