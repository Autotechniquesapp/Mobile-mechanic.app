...
async function syncSchedule(jobId){try{await invoke('calendar-sync',{action:'sync_job',job_id:jobId});}catch(e){console.warn('Calendar sync failed',e);}try{await action('zapier.emit',{event:'job_scheduled',payload:{job_id:jobId}});}catch{}}
