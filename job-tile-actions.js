...
function openNativeJob(id){close();if(typeof window.MobileMechanicOpenJob==='function')return window.MobileMechanicOpenJob(id);location.hash='#jobs';}
...
function deleteJob(id){if(!jobById(id))return toast('Job not found.','bad');const ok=window.confirm(`Permanently delete this job?\n\nThis removes the job and its local estimate, invoice, findings and work-order records from Mobile Mechanic AI. This cannot be undone.`);if(ok)dispatchPersistentAction('delete-job',id);}
