...
function deleteJob(id){if(!jobById(id))return toast('Job not found.','bad');const ok=window.confirm(`Remove this job from active jobs?\n\nUse this when the customer declined or does not want anything done. It will be retained as declined instead of being permanently erased.`);if(ok)dispatchPersistentAction('decline-job',id);}
