...
.mma-vehicle-card[role="button"]{cursor:pointer!important;touch-action:manipulation;}
.mma-vehicle-card[role="button"]:focus,.mma-vehicle-card[role="button"]:active{outline:2px solid #ef2a31;outline-offset:2px;}
...
function customerById(id){return shop()?.customers?.find(c=>String(c.id)===String(id))||null;}
function customerJobs(customer){
  if(!customer)return [];
  const phone=String(customer.phone||'').replace(/\D/g,'');
  const name=String(customer.name||'').trim().toLowerCase();
  return (shop()?.jobs||[]).filter(job=>{
    if(String(job.customerId||'')===String(customer.id))return true;
    const jobPhone=String(job.phone||job.intake?.phone||'').replace(/\D/g,'');
    const jobName=String(job.customerName||job.name||'').trim().toLowerCase();
...
function vehicleCard(j,jobId=''){
  ...
  const action=jobId?` role="button" tabindex="0" data-job-panel-open="${esc(jobId)}" aria-label="Open ${esc(vehicleText(v))} job"`:'';
  return `<div class="mma-vehicle-card"${action}><div class="mma-vehicle-icon">${ic('car')}</div><div class="mma-vehicle-main"><b class="mma-vehicle-title">${esc(vehicleText(v))}</b><div class="mma-vehicle-meta">${chips.length?chips.map(x=>`<span class="mma-vehicle-chip">${esc(x)}</span>`).join(''):'<span class="mma-vehicle-chip">Vehicle details pending</span>'}</div></div></div>`;
}
...
function interactiveValue(k,v){const key=String(k||'').toLowerCase(),text=String(v||'');if(key==='phone'){const dial=text.replace(/[^+\d]/g,'');return dial?`<a href="tel:${esc(dial)}">${esc(text)}</a>`:esc(text);}if(key==='email'&&text.includes('@'))return `<a href="mailto:${encodeURIComponent(text)}">${esc(text)}</a>`;if(/location|address/.test(key))return `<a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(text)}" target="_blank" rel="noopener">${esc(text)}</a>`;return esc(text);}
function intakeTable(j){const rows=intakeRows(j);if(!rows.length)return '<p>No intake answers saved on this job.</p>';return `<div class="table-wrap"><table class="estimate-table"><tbody>${rows.map(([k,v])=>`<tr><th>${esc(k)}</th><td>${interactiveValue(k,v)}</td></tr>`).join('')}</tbody></table></div>`;}
...
function dispatchPersistentAction(action,id){const button=document.createElement('button');button.type='button';button.hidden=true;button.dataset.action=action;button.dataset.job=String(id);document.body.appendChild(button);close();button.click();setTimeout(()=>button.remove(),0);}
function completeJob(id){if(!jobById(id))return toast('Job not found.','bad');dispatchPersistentAction('complete-job',id);}
function declineJob(id){if(!jobById(id))return toast('Job not found.','bad');dispatchPersistentAction('decline-job',id);}
function deleteJob(id){if(!jobById(id))return toast('Job not found.','bad');dispatchPersistentAction('delete-job',id);}
...
function openPanel(id){ensureStyles();const j=jobById(id);if(!j)return;setActive(j.id);close();const done=j.status==='Completed';const d=document.createElement('div');d.className='modal-backdrop';d.dataset.jobTileActionsModal='1';d.innerHTML=`<div class="modal" role="dialog" aria-modal="true" aria-label="Job intake and options"><div class="modal-head"><h2>${esc(j.customerName||'Customer Intake')}</h2><button class="close-btn" type="button" data-job-panel-close aria-label="Close">×</button></div><div class="job-banner" style="margin-bottom:10px"><div class="avatar">${esc(String(j.customerName||'J').split(/\s+/).map(x=>x[0]).join('').slice(0,2))}</div><div class="job-banner-main"><b class="mma-clickable-name" role="button" tabindex="0" data-job-panel-customer="${esc(j.customerId||'')}">${esc(j.customerName||'Customer')}</b><p>${esc(j.complaint||j.concern||j.description||'No complaint entered')}</p></div><span class="badge ${done?'green':'red'}">${esc(j.status||'Job')}</span></div>${vehicleCard(j,j.id)}<div class="work-card"><h3>Customer Intake Form</h3>${intakeTable(j)}</div><div class="btn-row" style="margin-top:10px"><button class="btn btn-primary" data-job-panel-open="${esc(j.id)}">${ic('wrench')} Open Full Job</button><button class="btn btn-soft" data-job-panel-schedule="${esc(j.id)}">${ic('calendar')} ${j.scheduledStart?'Edit Time':'Schedule'}</button><button class="btn btn-soft" data-job-panel-findings="${esc(j.id)}">Findings</button><button class="btn btn-soft" data-job-panel-estimate="${esc(j.id)}">${ic('money')} Estimate</button><button class="btn btn-soft" data-job-panel-maps="${esc(j.id)}">Google Maps</button>${done?'<span class="badge green">Already completed</span>':`<button class="btn btn-green" data-job-panel-complete="${esc(j.id)}">${ic('check')} Complete</button>`}<button class="btn btn-soft" data-job-panel-decline="${esc(j.id)}">Customer Declined</button><button class="btn btn-danger" data-job-panel-delete="${esc(j.id)}">Delete Job</button></div></div>`;document.body.appendChild(d);}
...
document.addEventListener('click',e=>{const customerTile=e.target.closest?.('[data-open-customer-intake]');if(customerTile){const nested=e.target.closest('button,a,input,select,textarea');if(!nested||nested===customerTile){e.preventDefault();e.stopImmediatePropagation();openCustomerIntake(customerTile.dataset.openCustomerIntake);return;}}const completedOpen=e.target.closest('[data-mma-completed-open]');if(completedOpen){e.preventDefault();openPanel(completedOpen.dataset.mmaCompletedOpen);return;}const closeBtn=e.target.closest('[data-job-panel-close]');if(closeBtn||e.target.matches('[data-job-tile-actions-modal]')){e.preventDefault();close();return;}const customer=e.target.closest('[data-job-panel-customer]');if(customer){e.preventDefault();e.stopPropagation();const id=customer.dataset.jobPanelCustomer;if(id)openCustomerIntake(id);return;}const del=e.target.closest('[data-job-panel-delete]');if(del){e.preventDefault();deleteJob(del.dataset.jobPanelDelete);return;}const decline=e.target.closest('[data-job-panel-decline]');if(decline){e.preventDefault();declineJob(decline.dataset.jobPanelDecline);return;}const complete=e.target.closest('[data-job-panel-complete]');if(complete){e.preventDefault();completeJob(complete.dataset.jobPanelComplete);return;}const schedule=e.target.closest('[data-job-panel-schedule]');if(schedule){e.preventDefault();openSchedule(schedule.dataset.jobPanelSchedule);return;}const mapsBtn=e.target.closest('[data-job-panel-maps]');if(mapsBtn){e.preventDefault();maps(jobById(mapsBtn.dataset.jobPanelMaps));return;}const full=e.target.closest('[data-job-panel-open],[data-job-panel-estimate]');if(full){e.preventDefault();openNativeJob(full.dataset.jobPanelOpen||full.dataset.jobPanelEstimate);return;}const findings=e.target.closest('[data-job-panel-findings]');if(findings){e.preventDefault();setActive(findings.dataset.jobPanelFindings);close();location.hash='#findings';return;}const tile=e.target.closest('[data-open-job],.mmp-job-row[data-job]');if(tile&&!tile.dataset.mmaNativeOpen){const id=tile.dataset.openJob||tile.dataset.job;const j=jobById(id);const control=e.target.closest('button,a,input,select,textarea');const nestedControl=control&&control!==tile;if(nameClickedInTile(e,tile,j)||!nestedControl){e.preventDefault();e.stopImmediatePropagation();openNativeJob(id);return;}}},true);
document.addEventListener('keydown',e=>{const customerLink=e.target.closest?.('[data-job-panel-customer],[data-job-panel-open]');if(customerLink&&(e.key==='Enter'||e.key===' ')){e.preventDefault();customerLink.click();return;}const customerTile=e.target.closest?.('[data-open-customer-intake]');if(customerTile&&(e.key==='Enter'||e.key===' ')){e.preventDefault();openCustomerIntake(customerTile.dataset.openCustomerIntake);return;}const tile=e.target.closest?.('[data-open-job],.mmp-job-row[data-job]');if(tile&&(e.key==='Enter'||e.key===' ')){e.preventDefault();openPanel(tile.dataset.openJob||tile.dataset.job);}},true);
let patchTimer=0;
