...
function markup(state){const {job,row,invoice,wo}=state;const complaint=row?.customer_states||job?.complaint||'';const codes=row?.codes||job?.codes||'';return `<section class="jwo" data-job-work-order>
  ...
  <div class="jwo-actions">${job?.status==='Completed'?'':`<button type="button" class="btn btn-green" data-action="complete-job" data-job="${esc(job?.id)}">Complete Job</button>`}<button type="button" class="btn btn-danger" data-action="delete-job" data-job="${esc(job?.id)}">Delete Job</button></div>
