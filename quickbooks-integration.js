...
function partsPortalMarkup(){return `<div class="section-note" style="margin:10px 0 4px">Parts ordering opens each supplier's secure website. Each shop signs in with its own commercial account; Mobile Mechanic AI does not store supplier passwords.</div>${PARTS_PORTALS.map((portal,index)=>`<div class="list-item"><div class="list-icon">${esc(portal.initials)}</div><div class="list-main"><b>${esc(portal.name)} <span class="badge green">Website</span></b><p>${esc(portal.note)}</p><div class="list-actions"><button class="btn btn-soft" data-parts-details="${index}">Connection Info</button><button class="btn btn-primary" data-open-parts-portal="${esc(portal.url)}">Open ${esc(portal.name)}</button></div></div></div>`).join('')}`;}
