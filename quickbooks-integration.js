...
function displayRow(row){if(row.provider==='google_maps')return {...row,name:'OpenStreetMap / Leaflet',status:'connected',configured:true,always_on:true,display_name:'Built-in free maps',last_error:null,note:'Free built-in maps, GPS location and nearby parts-store lookup. No Google Maps API key required.'};if(isZapierRow(row))return {...row,status:'optional',configured:true,last_error:null,note:'Optional automation service. Tap Learn More if this shop wants help connecting other apps.'};if(isOneDriveRow(row))return {...row,name:'Microsoft OneDrive',status:'optional',configured:true,last_error:null,display_name:null,note:'Optional Microsoft file storage for invoices, receipts and reports. Tap Learn More to request setup.'};return row;}
function badge(raw){const row=displayRow(raw);let status=row.status||'not_connected';if(status==='not_connected'&&!row.configured)status='needs_keys';const label={connected:'Connected',connecting:'Connecting',needs_keys:'Needs setup',needs_attention:'Needs attention',disabled:'Disabled',not_connected:'Not connected',optional:'Optional'}[status]||status;const cls=status==='connected'?'green':status==='needs_attention'||status==='optional'?'orange':'red';return `<span class="badge ${cls}">${esc(label)}</span>`;}
