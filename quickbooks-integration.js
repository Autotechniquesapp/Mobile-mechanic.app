...
function portalSettings(){try{return JSON.parse(localStorage.getItem(portalKey())||'{}');}catch{return {};}}
function portalEnabled(provider){return portalSettings()[provider]!==false;}
function setPortalEnabled(provider,enabled){const settings=portalSettings();settings[provider]=enabled;localStorage.setItem(portalKey(),JSON.stringify(settings));}
...
function displayRow(row){
  if(row.provider==='google_maps')return {...row,name:'OpenStreetMap / Leaflet',status:'connected',configured:true,always_on:true,display_name:'Built-in free maps',last_error:null,note:'Free built-in maps, GPS location and nearby parts-store lookup. No Google Maps API key required.'};
  if(row.provider==='carfax')return {...row,name:'CARFAX',status:'under_review',configured:true,last_error:null,note:'We are looking into connecting to CARFAX.'};
  if(isZapierRow(row))return {...row,status:'optional',configured:true,last_error:null,note:'Optional automation service. Tap Learn More if this shop wants help connecting other apps.'};
  if(row.provider==='microsoft_calendar')return {...row,name:'Microsoft 365 / Outlook Calendar',note:'Sync scheduled jobs to the shop\'s Outlook / Microsoft 365 calendar.'};
  if(row.provider==='microsoft_email')return {...row,name:'Microsoft 365 / Outlook Email',note:'Send estimates, invoices and follow-ups from the shop\'s Outlook / Microsoft 365 mailbox.'};
  if(row.provider==='onedrive')return {...row,name:'Microsoft OneDrive',note:'Back up completed service records, invoices, receipts and reports to the shop\'s Microsoft OneDrive.'};
  return row;
}
...
function rowMarkup(raw){const row=displayRow(raw);return `<div class="list-item"><div class="list-icon">${esc((row.name||'?').split(/\s+/).map(x=>x[0]).join('').slice(0,2).toUpperCase())}</div><div class="list-main"><b>${esc(row.name)} ${badge(row)}</b><p>${esc(row.note||'Business service integration.')}</p>${row.display_name?`<p class="small muted">Connected account: ${esc(row.display_name)}</p>`:''}${row.last_error?`<p class="small red">${esc(row.last_error)}</p>`:''}<div class="list-actions"><button class="btn btn-soft" data-business-details="${esc(row.provider)}">View Status</button>${actionButton(row)}${row.provider==='quickbooks'&&row.status==='connected'?'<button class="btn btn-soft" data-quickbooks-check>Check Company</button>':''}</div></div></div>`;}
function partsPortalMarkup(){return `<div class="section-note" style="margin:10px 0 4px">Parts ordering opens each supplier's secure website. Each shop signs in with its own commercial account; Mobile Mechanic AI does not store supplier passwords.</div>${PARTS_PORTALS.map((portal,index)=>{const enabled=portalEnabled(portal.provider);return `<div class="list-item"><div class="list-icon">${esc(portal.initials)}</div><div class="list-main"><b>${esc(portal.name)} <span class="badge ${enabled?'green':'red'}">${enabled?'Enabled':'Disabled'}</span></b><p>${esc(portal.note)}</p><div class="list-actions"><button class="btn btn-soft" data-parts-details="${index}">Connection Info</button><button class="btn ${enabled?'btn-soft':'btn-primary'}" data-parts-toggle="${esc(portal.provider)}">${enabled?'Disable':'Enable'}</button>${enabled?`<button class="btn btn-primary" data-open-parts-portal="${esc(portal.url)}">Open ${esc(portal.name)}</button>`:''}</div></div></div>`;}).join('')}`;}
...
function statusLabel(row){const status=row.status||'not_connected';return {connected:'Connected',connecting:'Connecting',needs_keys:'Needs setup',needs_attention:'Needs attention',disabled:'Disabled',not_connected:'Not connected',optional:'Optional',under_review:'Under review'}[status]||status;}
function detailPage(row){const body=document.querySelector('[data-business-integrations-body]');if(!body)return;const issue=row.last_error?`<div class="alert bad"><b>Problem found</b><br>${esc(row.last_error)}</div>`:`<div class="alert good"><b>No reported connection error</b><br>${row.status==='connected'?'The provider reports this shop as connected.':'This provider is not currently connected.'}</div>`;body.innerHTML=`<button class="btn btn-soft" data-business-details-back>‹ All Integrations</button><div class="list-item" style="margin-top:10px"><div class="list-icon">${esc((row.name||'?').split(/\s+/).map(x=>x[0]).join('').slice(0,2).toUpperCase())}</div><div class="list-main"><h3 style="margin:0 0 5px">${esc(row.name)}</h3><p><b>Status:</b> ${esc(statusLabel(row))}<br><b>Setup:</b> ${row.configured?'Provider credentials are installed':'Provider credentials or approval are still required'}${row.display_name?`<br><b>Account:</b> ${esc(row.display_name)}`:''}</p><p>${esc(row.note||'Business service integration.')}</p>${issue}<div class="list-actions">${actionButton(row)}</div></div></div>`;}
async function openDetails(provider){if(busy)return;busy=true;try{const d=await invoke('business-integrations',{action:'status'}),raw=(d.integrations||[]).find(x=>x.provider===provider);if(!raw)throw new Error('That integration is not available for this shop.');detailPage(displayRow(raw));}catch(err){toast(err.message||'Could not check integration status.','bad');}finally{busy=false;}}
function partsDetail(index){const portal=PARTS_PORTALS[Number(index)],body=document.querySelector('[data-business-integrations-body]');if(!portal||!body)return;const enabled=portalEnabled(portal.provider);body.innerHTML=`<button class="btn btn-soft" data-business-details-back>‹ All Integrations</button><div class="list-item" style="margin-top:10px"><div class="list-icon">${esc(portal.initials)}</div><div class="list-main"><h3 style="margin:0 0 5px">${esc(portal.name)}</h3><p><b>Shop setting:</b> ${enabled?'Enabled':'Disabled'}<br><b>Account check:</b> Sign in on the supplier website to verify this shop's commercial account.</p><p>Mobile Mechanic AI does not store the supplier password. Automatic inventory, pricing, cart transfer, or order status requires approval and API access from ${esc(portal.name)}.</p><div class="alert good"><b>No Mobile Mechanic AI connection error</b><br>The app can open the secure supplier login. Login or account errors must be resolved with the supplier.</div><div class="list-actions"><button class="btn ${enabled?'btn-soft':'btn-primary'}" data-parts-toggle="${esc(portal.provider)}">${enabled?'Disable':'Enable'}</button>${enabled?`<button class="btn btn-primary" data-open-parts-portal="${esc(portal.url)}">Open ${esc(portal.name)}</button>`:''}</div></div></div>`;}
...
window.MobileMechanicOpenBusinessIntegrations=openIntegrationsPage;
document.addEventListener('click',e=>{
  ...
  const partsInfo=e.target.closest?.('[data-parts-details]');if(partsInfo){e.preventDefault();partsDetail(partsInfo.dataset.partsDetails);return;}
  const partsToggle=e.target.closest?.('[data-parts-toggle]');if(partsToggle){e.preventDefault();const provider=partsToggle.dataset.partsToggle;setPortalEnabled(provider,!portalEnabled(provider));toast(`${PARTS_PORTALS.find(x=>x.provider===provider)?.name||'Parts integration'} ${portalEnabled(provider)?'enabled':'disabled'} for this shop.`,'good');renderIntegrations();return;}
  const details=e.target.closest?.('[data-business-details]');if(details){e.preventDefault();openDetails(details.dataset.businessDetails);return;}
  if(e.target.closest?.('[data-business-details-back]')){e.preventDefault();renderIntegrations();return;}
