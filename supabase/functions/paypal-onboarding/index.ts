...
Deno.serve(async(req)=>{
  ...
  async function platformToken(){
    ...
    if(!r.ok||!d.access_token)throw new Error(d?.error_description||d?.error||"Could not authenticate with PayPal.");
  ...
  try{
    if(req.method==="GET"){
      ...
      const headers:any={Authorization:`Bearer ${access}`,Accept:"application/json","PayPal-Auth-Assertion":paypalAssertion(clientId,merchantId)};
      if(bnCode)headers["PayPal-Partner-Attribution-Id"]=bnCode;

      // Verify seller identity and permissions against PayPal rather than trusting callback flags.
      ...
      const oauthParties=(Array.isArray(seller.oauth_integrations)?seller.oauth_integrations:[])
        .filter((integration:any)=>integration.integration_type==="OAUTH_THIRD_PARTY"&&(!integration.status||integration.status==="A")&&Array.isArray(integration.oauth_third_party))
        .flatMap((integration:any)=>integration.oauth_third_party)
        .filter((party:any)=>party.partner_client_id===clientId);
      const scopes=new Set(oauthParties.flatMap((party:any)=>Array.isArray(party.scopes)?party.scopes:[]));
      const paymentPermitted=scopes.has("https://uri.paypal.com/services/payments/payment/authcapture");
      const invoicePermitted=scopes.has("https://uri.paypal.com/services/invoicing/invoices/readwrite")||scopes.has("https://api.paypal.com/v1/invoicing:*");
      const identityMatches=result.ok&&seller.merchant_id===merchantId&&seller.tracking_id===state;
      if(!identityMatches||!paymentPermitted){
      ...
      const primaryEmail=String(seller.primary_email||"").trim();
      const ready=seller.payments_receivable===true&&seller.primary_email_confirmed===true;
      const invoiceReady=ready&&invoicePermitted&&!!primaryEmail;
      const displayName=String(seller.legal_name||[seller.given_name,seller.surname].filter(Boolean).join(" ")||"PayPal").trim();

      await checked(admin.from("payment_processor_credentials").upsert({
        shop_id:st.shop_id,
        provider:"paypal",
        ...
        credential_metadata:{
          ...
          email_confirmed:seller.primary_email_confirmed===true,
          payments_receivable:seller.payments_receivable===true,
          invoice_permission:invoicePermitted,
          seller_products:Array.isArray(seller.products)?seller.products:[]
        },
        updated_at:new Date().toISOString()
      },{onConflict:"shop_id,provider"}));

      await checked(admin.from("shop_payment_processors").update({
        status:ready?"connected":"needs_attention",
        external_account_id:merchantId,
        display_name:displayName||"PayPal",
        public_settings:{mode:env,email_confirmed:seller.primary_email_confirmed===true},
        capabilities:{paypal:ready,venmo:false,invoices:invoiceReady},
        last_error:ready?(invoiceReady?null:"PayPal payments are connected. PayPal invoicing permission is not active for this seller."):"Complete PayPal account verification before accepting payments.",
        last_synced_at:new Date().toISOString(),
        updated_at:new Date().toISOString()
      }).eq("shop_id",st.shop_id).eq("provider","paypal"));
      return Response.redirect(back,302);
    }
    ...
    const access=await platformToken();
    const headers:any={Authorization:`Bearer ${access}`,"Content-Type":"application/json"};
    if(bnCode)headers["PayPal-Partner-Attribution-Id"]=bnCode;
    const referral={
      tracking_id:state,
    ...
    const d=await r.json();
    if(!r.ok)throw new Error(d?.message||d?.details?.[0]?.description||"PayPal seller onboarding could not be started.");
