...
test('Autotechniques clean intake route has complete message-preview metadata', () => {
  ...
  assert.match(page, /property="og:image" content="https:\/\/mobile-mechanic\.app\/assets\/autotechniques-intake-preview\.jpg\?v=20260917-intakefix"/);
  assert.match(page, /name="twitter:card" content="summary_large_image"/);
});

test('Autotechniques clean intake route is standalone and submits directly', () => {
  assert.doesNotMatch(page, /location\.replace\(/);
  assert.match(page, /id="intakeForm"/);
  assert.match(page, /submit_public_intake/);
  assert.match(page, /SHOP_ID='shp_autotechniques'/);
  assert.match(page, /Autotechniques/);
});
