...
test('Square invoice authorization follows the invoice shop, not the oldest membership',()=>{
  assert.match(square,/eq\("shop_id",invoice\.shop_id\)/);
  assert.doesNotMatch(square,/eq\("status","active"\)\.limit\(1\)\.maybeSingle\(\)/);
  assert.match(square,/Invoice not found/);
});

test('public estimate backend is tracked for approval-flow review',()=>{
